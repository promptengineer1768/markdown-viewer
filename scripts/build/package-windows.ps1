...
function Assert-PrebuiltReleaseArtifacts {
    ...
    $mainExe = Join-Path $BuildBinDir "markdown_viewer.exe"
    if (-not (Test-Path $mainExe)) {
        throw "Release executable not found: $mainExe. Run build-msvc-release.bat first."
    }

    $localeRoot = Join-Path $BuildBinDir "locales"
    if (-not (Test-Path $localeRoot)) {
        throw "Locales directory not found: $localeRoot. Build may be incomplete."
    }
}
...
function Invoke-InnoSetup {
    $issFile = Join-Path $ProjectRoot "installer\markdown-viewer.iss"
